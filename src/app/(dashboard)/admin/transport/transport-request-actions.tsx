...
import { processTransportRequest } from "@/app/actions/enterprise"
...
export function TransportRequestActions({
  requestId,
  studentName,
  requestedRouteName,
  ...
  function handleApprove() {
    ...
    startTransition(async () => {
      const res = await processTransportRequest(requestId, "APPROVED")
      if (res.success) {
        router.refresh()
      } else {
  ...
  function handleRejectSubmit(e: React.FormEvent) {
    ...
    setErrorMessage(null)
    startTransition(async () => {
      const res = await processTransportRequest(
        requestId,
        "REJECTED",
      ...
      {!showRejectInput ? (
        ...
        <form onSubmit={handleRejectSubmit} className="flex flex-col gap-2 p-2 bg-rose-50/60 rounded-lg border border-rose-200">
          ...
          <input
            type="text"
            required
            placeholder="e.g. Route capacity full, Invalid address"
